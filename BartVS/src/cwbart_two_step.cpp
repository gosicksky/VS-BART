...
//[[Rcpp::export]]
RcppExport SEXP cwbart_two_step(
    SEXP sexp_in,
    SEXP sexp_ip,
    SEXP sexp_inp,
    SEXP sexp_ix,
...
//   Rcpp::List list_of_lists(nkeeptreedraws*treesaslists);
   ...
   Rcpp::NumericMatrix gamma_save(nd, 1);
   ...
   //Z_Gamma is a matrix 1*1 with value 1
   z_Gamma[0][0] = 1;
   ...
   for(size_t i=0;i<total;i++) {
      ...
      if(i>=burn) {
        ...
        // }
        for (int j = 0; j < group_num; j++) {
          b_save(i-burn, j) = z_b[j][0];
        }
        int gamma_index = 0;
        gamma_save(i-burn, 0) = z_Gamma[0][0];
        for (int j = 0; j < q; j++) {
