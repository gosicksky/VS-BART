...
public:
   ...
   friend bool bd(tree& x, xinfo& xi, dinfo& di, pinfo& pi, double sigma,
		  std::vector<size_t>& nv, std::vector<double>& pv, bool aug, rn& gen, std::vector<std::vector<double>>& mr_vec);
   //------------------------------
   ...
   void draw(double sigma, std::vector<double> rf_result, rn& gen);
//   void draw_s(rn& gen);
