...
wbart_unified = function(x.train, 
  ...
  #--------------------------------------------------
  print("Calculate start point")
  start_point_indicator = 0
  nu = sigdf
  if(is.na(lambda)) {
    if(is.na(sigest)) {
      if(p < N & q * length(unique(id)) < N) {
        start_point_indicator = 1
  ...
  print("start point calculated")
  
  #--------------------------------------------------
  
  #random effect
  if (start_point_indicator == 1) {
  ...
  if(length(grp) == length(unique(grp))) {
    ...
    varcount = matrix(0, nrow = nkeeptreedraws, ncol = p0)
    varprob = matrix(0, nrow = nkeeptreedraws, ncol = p0)
    ...
    ## vip
    #View(t(apply(varcount, 1, function(s) s / sum(s))))
...
wbart_two_step = function(x.train, 
  ...
  if(!transposed) {
    ...
    z_temp = rfModelMatrix_two_step(z.train)
    z.train = t(z_temp$Z)
    z_grp = z_temp$grp
