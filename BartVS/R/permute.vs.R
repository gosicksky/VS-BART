...
permute.vs_unified = function(x.train, 
  ...
  if(ncol(rfModelMatrix_unified(z.train)$Z) > ncol(z.train))
    avg.lambda.mtx = matrix(0, nrow = nreps, ncol = 2 ^ (ncol(z.train) + 1))
  else
    avg.lambda.mtx = matrix(0, nrow = nreps, ncol = 2 ^ ncol(z.train))
  ...
  while (cnt < nreps) {
    print(cnt)
    bart = wbart_unified(x.train = x.train, y.train = y.train, z.train = z.train, id = id,
                 sparse = FALSE,
    ...
    for(row in 1:nrow(combined.lambda)) {
      #combine each row value together and treat them as binary and convert to decimal
      bin_str <- paste(rev(combined.lambda[row,]), collapse = "")
      binary_to_decimal <- strtoi(bin_str, base = 2)
      avg.lambda.mtx[cnt, binary_to_decimal + 1] = avg.lambda.mtx[cnt, binary_to_decimal + 1] + 1
    }
    avg.lambda.mtx[cnt, ] = avg.lambda.mtx[cnt, ] / nrow(bart$combined.lambda)
  ...
  if(ncol(rfModelMatrix_unified(z.train)$Z) > ncol(z.train))
    permute.lambda = matrix(0, nrow = npermute, ncol = 2 ^ (ncol(z.train) + 1))
  else
    permute.lambda = matrix(0, nrow = npermute, ncol = 2 ^ ncol(z.train))
  ...
  while (cnt < npermute) {
    ...
    #follow the same variable type as z.train
    for (col in seq_along(z.permuted)) {
      if (is.factor(z.train[[col]])) {
        z.permuted[[col]] <- factor(z.permuted[[col]])
      } else if (is.character(z.train[[col]])) {
        z.permuted[[col]] <- as.character(z.permuted[[col]])
      }
    }
    print("shuffle complete")
    
    bart = wbart_unified(x.train = x.train, y.train = y.permuted, z.train = z.permuted, id = id,
                 sparse = FALSE,
                 xinfo = xinfo, numcut = numcut, usequants = usequants, cont = cont, rm.const = rm.const,
                 k = k, power = power, base = base, split.prob = split.prob,
                 ntree = ntree, ndpost = ndpost, nskip = nskip, keepevery = keepevery,
    ...
    for(row in 1:nrow(combined.lambda)) {
      ...
      bin_str <- paste(rev(combined.lambda[row,]), collapse = "")
      binary_to_decimal <- strtoi(bin_str, base = 2)
      permute.lambda[cnt, binary_to_decimal + 1] = permute.lambda[cnt, binary_to_decimal + 1] + 1
    }
    permute.lambda[cnt, ] = permute.lambda[cnt, ] / nrow(bart$combined.lambda)
  ...
  rf_number = ncol(rfModelMatrix_unified(z.train)$Z)
  rf_select = as.binary(which(avg.lambda == max((avg.lambda)[avg.lambda > lambda.pointwise.cutoffs])) - 1)
  rf_select = rev(c(rep(0, rf_number - length(rf_select)), rf_select))
...
permute.vs_two_step = function(x.train, 
  ...
  #calculate the average for each lambda to be zero, and if x1 is significant, and x2,x3,x4 not, then the 9th element will be the largest
  #if x1,x3 is significant, and x2,x4 not, then the 11th element will be the largest
  if(ncol(rfModelMatrix_two_step(z.train)$Z) > ncol(z.train))
  ...
  while (cnt < nreps) {
    print(cnt)
    bart = wbart_two_step(x.train = x.train, y.train = y.train, z.train = z.train, id = id,
                 sparse = FALSE,
                 xinfo = xinfo, numcut = numcut, usequants = usequants, cont = cont, rm.const = rm.const,
                 k = k, power = power, base = base, split.prob = split.prob,
                 ntree = ntree, ndpost = ndpost, nskip = nskip, keepevery = keepevery,
  ...
  while (cnt < npermute) {
    ...
    bart = wbart_two_step(x.train = x.train, y.train = y.permuted, z.train = z.train, id = id,
                 sparse = FALSE,
                 xinfo = xinfo, numcut = numcut, usequants = usequants, cont = cont, rm.const = rm.const,
                 k = k, power = power, base = base, split.prob = split.prob,
                 ntree = ntree, ndpost = ndpost, nskip = nskip, keepevery = keepevery,
